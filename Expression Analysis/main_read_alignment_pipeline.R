...
if (mode == "pathway"){
  GO_id <- args[6]
  dir.create(file.path(main_directory, gsub(':', '_', GO_id)))
  dir.create(file.path(main_directory, gsub(':', '_', GO_id), paste0("threshold_", gsub('[.]', '_', threshold))))
  directory <- file.path(main_directory, gsub('[:]', '_', GO_id), paste0("threshold_", gsub('[.]', '_', threshold)))
} else if (mode == "set"){
...
abundance_no_zeros1 <- abundance[abundance[,1] != 0 & abundance[,2] != 0, ]
abundance_no_zeros2 <- abundance[abundance[,1] != 0 & abundance[,3] != 0, ]
abundance_no_zeros3 <- abundance[abundance[,1] != 0 & abundance[,4] != 0, ]

abundance_no_zeros1 <- abundance_no_zeros1[abundance_no_zeros1[,1] > 1 | abundance_no_zeros1[,2] > 1, ]
abundance_no_zeros2 <- abundance_no_zeros2[abundance_no_zeros2[,1] > 1 | abundance_no_zeros2[,3] > 1, ]
abundance_no_zeros3 <- abundance_no_zeros3[abundance_no_zeros3[,1] > 1 | abundance_no_zeros3[,4] > 1, ]

#graphing LogFoldChange for all genes
min_CCM1 <- -ceiling(abs(min(abundance_no_zeros1$logFoldChangeCCM1)))
max_CCM1 <- ceiling(max(abundance_no_zeros1$logFoldChangeCCM1))
min_CCM2 <- -ceiling(abs(min(abundance_no_zeros2$logFoldChangeCCM2)))
max_CCM2 <- ceiling(max(abundance_no_zeros2$logFoldChangeCCM2))
min_CCM3 <- -ceiling(abs(min(abundance_no_zeros3$logFoldChangeCCM3)))
max_CCM3 <- ceiling(max(abundance_no_zeros3$logFoldChangeCCM3))

own_graph_CCM1 = ggplot(data = abundance_no_zeros1, aes(x = logFoldChangeCCM1)) + geom_histogram(binwidth = 0.1) + scale_x_continuous(breaks = seq(min_CCM1,max_CCM1), limits=c(min_CCM1,max_CCM1))
#own_graph_CCM1
ggsave(file.path(directory, paste("own_graph_CCM1", label, ".png", sep = "")))

own_graph_CCM2 = ggplot(data = abundance_no_zeros2, aes(x = logFoldChangeCCM2)) + geom_histogram(binwidth = 0.1) + scale_x_continuous(breaks = seq(min_CCM2,max_CCM2), limits=c(min_CCM2,max_CCM2))
#own_graph_CCM2
ggsave(file.path(directory,paste("own_graph_CCM2", label, ".png", sep = "")))

own_graph_CCM3 = ggplot(data = abundance_no_zeros3, aes(x = logFoldChangeCCM3)) + geom_histogram(binwidth = 0.1) + scale_x_continuous(breaks = seq(min_CCM3,max_CCM3), limits=c(min_CCM3,max_CCM3))
#own_graph_CCM3
ggsave(file.path(directory,paste("own_graph_CCM3", label, ".png", sep = "")))


#differentially expressed genes
differential_gene_expression_CCM1 <- abundance_no_zeros1[abs(abundance_no_zeros1$logFoldChangeCCM1) > threshold, ]
differential_gene_expression_CCM2 <- abundance_no_zeros2[abs(abundance_no_zeros2$logFoldChangeCCM2) > threshold, ]
differential_gene_expression_CCM3 <- abundance_no_zeros3[abs(abundance_no_zeros3$logFoldChangeCCM3) > threshold, ]

differential_gene_expression <- unique(rbind(differential_gene_expression_CCM1, differential_gene_expression_CCM2, differential_gene_expression_CCM3))
...
rownames(abundance_no_zeros1) <- gsub("\\..*","",rownames(abundance_no_zeros1))
rownames(abundance_no_zeros2) <- gsub("\\..*","",rownames(abundance_no_zeros2))
rownames(abundance_no_zeros3) <- gsub("\\..*","",rownames(abundance_no_zeros3))
...
if (mode == "pathway") {
  ...
  gene_expression_CCM1 = lapply(pathway_ensembl, function(x){abundance_no_zeros1[rownames(abundance_no_zeros1) == x, 5]})
  gene_expression_CCM2 = lapply(pathway_ensembl, function(x){abundance_no_zeros2[rownames(abundance_no_zeros2) == x, 6]})
  gene_expression_CCM3 = lapply(pathway_ensembl, function(x){abundance_no_zeros3[rownames(abundance_no_zeros3) == x, 7]})
  ...
  i <- 1
  for (gene in row.names(result_table)) {
    g <- ggplot(data = melt(result_table[gene, 1:3]), aes(x = variable, y = value)) + geom_bar(stat="identity") + labs(x = result_table[gene,5], y = "")  + theme(axis.text.x = element_text(angle = 45, colour="grey20",size=4),axis.text.y = element_text(colour="grey20",size=4))
    plots[[i]]<-g
    i <- i+1
  }
  ...
  ml
  ggsave(file.path(directory, paste("graphs", 1, "-", length(row.names(result_table)), label, "_", gsub('[.]', '_', threshold), ".png",sep="")), ml, width = 30, height = 25, units = c("cm"), dpi = 1000)
  ...
  for (gene in row.names(wanted_genes_matrix)) {
    ...
    g <- ggplot(data = single_matrix, aes(x = variable, y = value, fill = compare)) + geom_bar(stat="identity") + labs(x = wanted_genes_matrix[gene,5][[1]], y = "")  + theme(axis.text.x = element_text(angle = 45, colour="grey20",size=4),axis.text.y = element_text(colour="grey20",size=4),legend.position="none") + scale_fill_manual(name="Importance", labels = c("doubtful", "improtant"), values = c("doubtful"="grey", "improtant"="green"))
    wanted_plots[[i]]<-g
    i <- i+1
  }
  ...
  wanted
  ggsave(file.path(directory, paste("wanted_genes", label, ".png",sep="")), wanted, width = 30, height = 25, units = c("cm"), dpi = 1000)
}
